...
const mapStateToProps = (state) => ({
  rockSongs: state.home.rockSongs,
  popSongs: state.home.popSongs,
  hipHopSongs: state.home.rockSongs,
})

const mapDispatchToProps = (dispatch) => ({
  getArtist: (artistName, category) => {
    dispatch(getArtistAction(artistName, category))
  },
})
...
class Home extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    for (let j = 0; j < rockRandomArtists.length; j++)
      await this.props.getArtist(rockRandomArtists[j], "rockSongs")

    for (let k = 0; k < popRandomArtists.length; k++)
      await this.props.getArtist(popRandomArtists[k], "popSongs")

    for (let l = 0; l < hipHopRandomArtists.length; l++)
      await this.props.getArtist(hipHopRandomArtists[l], "hipHopSongs")
  }
